...
{
    int result = xum1541_read_ext_cb((struct opencbm_usb_handle *)HandleDevice, XUM1541_TAP, Buffer, Buffer_Length, Status, BytesRead, ReadCallback);
    if (result <= 0) {
...
{
    int result = xum1541_write_ext_cb((struct opencbm_usb_handle *)HandleDevice, XUM1541_TAP, Buffer, Length, Status, BytesWritten, WriteCallback);
    if (result <= 0) {
